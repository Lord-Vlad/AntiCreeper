...
execute if score @e[tag=acInRange,scores={acFuse=0},limit=1] acFuse matches 0 unless entity @e[type=creeper,distance=..4] run schedule function ac:gamerule 1s replace
execute if score @e[tag=acInRange,scores={acFuse=0},limit=1] acFuse matches 0 if entity @e[type=creeper,distance=4.1..] run schedule function ac:gamerule 10s replace
